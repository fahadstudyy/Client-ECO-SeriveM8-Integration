...
def handle_job_activity(data):
    ...
    elif current_stage == READY_TO_SCHEDULE_STAGE_ID:
        logging.info(f"Updating deal {deal_id} to Job Scheduled stage")
        update_hubspot_deal(deal_id, {"pre_install_inspection_date": formatted_date})
        update_hubspot_deal_stage(deal_id, JOB_SCHEDULED_PIPELINE_ID)
